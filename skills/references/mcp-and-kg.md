...
| **Knowledge CI** | `wiki_lint`, `wiki_compile` — gated by **`mcp.compile_enabled`** (default false); site rebuild also needs **`mcp.compile_allow_site`** |
...
**Knowledge compiler (`compile.*` / `knowledge_graph.*`):** Prefer **`llm-wiki compile`** / MCP **`wiki_compile`** (requires **`mcp.compile_enabled`**) after wiki merge. **`knowledge_graph.aliases`** merges entity spellings on add/query/rebuild. Non-empty **`allowed_predicates`** rejects unknown predicates on **`kg add`** (builtins included unless **`ontology_strict`**; structural `mentions`/`links_to`/`tagged` always allowed). Lint/compile write **`outputs/claims.json`** when **`extract_claims`**. Surgical: **`compile --raw raw/foo.md`**. Opt-in stubs: **`compile --stubs`** → **`outputs/stubs/`** only.
