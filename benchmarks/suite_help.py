...
**Peer backends (optional, same LME JSON):** `benchmark run lme --peer mem0` (repeat `--peer` for several). Installs and vector data live under **`benchmark.peers.cache_dir`** (default `~/.cache/llm-wiki-benchmarks/peers`, not tracked in git). Peers: **mem0** (pip `mem0ai`), **mempalace** / **claude-mem** (optional `MEMPALACE_BENCH_CMD` / `CLAUDE_MEM_BENCH_CMD` JSON stdin→stdout scripts), **supermemory** (stub until a client is wired). Dimension scores (Data Integrity, Simplicity, Integration, Arch Maturity) merge **proxies** with **`benchmarks/peers/rubric_overrides.json`**.
